    ...
    <div class="content">
        <div class="container mt-5 pt-5">
            <div class="form-container">
                <h2 class="text-center mb-4">Edit Data Feedback</h2>
                <form action="{{ route('awal.update', $data_awal->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group row">
                        <label for="nama_lengkap" class="col-md-4 col-form-label">Nama Lengkap</label>
                        <div class="col-md-8">
                            <input id="nama_lengkap" name="nama_lengkap" type="text" class="form-control" value="{{ old('nama_lengkap', $data_awal->nama_lengkap) }}" required>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-8">
                            <input id="no_tlp" name="no_tlp" type="number" class="form-control" value="{{ old('no_tlp', $data_awal->no_tlp) }}" required>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-8">
                            <input id="email_aktif" name="email_aktif" type="email" class="form-control" value="{{ old('email_aktif', $data_awal->email_aktif) }}" required>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-8">
                            <select id="select" name="select" class="custom-select" aria-describedby="selectHelpBlock">
                                <option value="⭐" {{ old('select', $data_awal->rating) == '⭐' ? 'selected' : '' }}> ⭐</option>
                                <option value="⭐⭐" {{ old('select', $data_awal->rating) == '⭐⭐' ? 'selected' : '' }}> ⭐⭐</option>
                                <option value="⭐⭐⭐" {{ old('select', $data_awal->rating) == '⭐⭐⭐' ? 'selected' : '' }}> ⭐⭐⭐</option>
                                <option value="⭐⭐⭐⭐" {{ old('select', $data_awal->rating) == '⭐⭐⭐⭐' ? 'selected' : '' }}> ⭐⭐⭐⭐</option>
                                <option value="⭐⭐⭐⭐⭐" {{ old('select', $data_awal->rating) == '⭐⭐⭐⭐⭐' ? 'selected' : '' }}> ⭐⭐⭐⭐⭐</option>
                            </select>
                            <span id="selectHelpBlock" class="form-text text-muted">Tolong masukan rating bintang sesuai kepuasan anda!</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="pesan_saran" class="col-md-4 col-form-label">Pesan/Saran (Opsional)</label>
                        <div class="col-md-8">
                            <textarea id="pesan_saran" name="pesan_saran" class="form-control">{{ old('pesan_saran', $data_awal->pesan_saran) }}</textarea>
                            <small class="form-text text-muted">Masukan Pesan/Saran Anda terkait pelayanan atau website Resmi TernakPro!!</small>
                        </div>
                    </div>
                    <div class="form-group row">
